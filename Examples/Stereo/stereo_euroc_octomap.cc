...
int main(int argc, char **argv)
{
    if(argc != 6)
    {
        cerr << endl << "Usage: ./stereo_euroc_octomap path_to_vocabulary path_to_settings path_to_left_folder path_to_right_folder path_to_times_file" << endl;
        return 1;
    }
    ...
    if(!bUseViewer)
    {
        cerr << "[stereo_euroc_octomap] DISPLAY is not set, running without Pangolin/OpenCV viewer." << endl;
    }
    ...
    for(int ni = 0; ni < nImages; ++ni)
    {
        ...
        const double tframe = vTimeStamp[ni];
